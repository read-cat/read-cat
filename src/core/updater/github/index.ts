...
export class GithubUpdater implements Updater {
  async getUpdateLog(version: string): Promise<Version | null> {
    ...
    const release = (<Release[]>body).find(r => r.tag_name === `v${version}`);
    return !release ? null : {
  ...
  async getNewVersion(branch: 'dev' | 'release'): Promise<Version | null> {
    ...
    if (process.platform === 'win32') {
      const asset = release?.assets.find(a => a.name.endsWith('.exe') && a.name.includes(`${process.platform}-${os.arch()}`));
